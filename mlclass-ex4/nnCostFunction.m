...
%%%BEGIN CALCULATIONS FOR GRADIENTS%%%%%%
delta3 = yy-a3; 

delta2 = (Theta2' * delta3') .* sigmoidGradient([ones(1,m);z2]);

%there's no computing for delta1 



D2 = Theta2_grad + (delta3'*a2);

test = delta2*a1;
delta2Trimmed = delta2(2:end,:); 


D1 = Theta1_grad + (delta2Trimmed*a1);
...
aux1=Theta1(:,2:end);
rowsTheta1 = size(Theta1,1);
aux1 = [zeros(rowsTheta1,1) aux1];


aux2 = Theta2(:,2:end);
rowsTheta2 = size(Theta2,1);
aux2 = [zeros(rowsTheta2,1) aux2];





Theta1_grad = D1 + ( (lambda/m)* aux1 ) ;
Theta2_grad = D2 + ( (lambda/m)* aux2 ) ;
...
%%%%end CALCULATIONS FOR GRADIENTS%%%%%%%
